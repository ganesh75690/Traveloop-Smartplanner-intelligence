...
import { 
  ArrowLeft, Plus, Trash2, RotateCcw, Package, 
  Sparkles, CloudSun, Thermometer, Droplets, AlertTriangle,
  ChevronDown, ChevronUp, Weight, Clock, Users, Mic,
  Lightbulb, Shield, Save, X, CheckCircle, AlertCircle,
  Plane, Sun, Mountain, Coffee, Baby, User, MapPin, Calendar
} from "lucide-react";
import useTrips from "../hooks/useTrips";
import { useTheme } from "../context/ThemeContext";
import { mockPackingItems } from "../data/mockData";

// TODO: replace with packingAPI.getAll(tripId)
...
export default function PackingChecklistPage() {
  ...
  const [items, setItems] = useState(mockPackingItems.filter((p) => p.tripId === tripId || tripId === "t1"));
    ...
    <div className="min-h-screen bg-[#EEF2FF] p-4 md:p-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div className="flex items-center gap-3">
          <button onClick={() => navigate(-1)} className={`p-2 rounded-xl ${bgClass} ${borderClass} ${mutedClass} hover:text-[#006CE4] hover:border-[#006CE4]/30 transition-all shadow-sm`}>
            <ArrowLeft size={18} />
          </button>
          <div>
            ...
            <p className="text-sm text-(--text-muted)">{trip?.name || "Trip"}</p>
          </div>
        </div>
